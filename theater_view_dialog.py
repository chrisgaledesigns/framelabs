...
class TheaterViewDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        """Build the image label, frame-position label, scrub bar, and
        Close button.
        """
        ...
        self._scrub_bar.sliderMoved.connect(self.go_to_index)

        bottom_bar = QWidget()
        bottom_bar_layout = QHBoxLayout(bottom_bar)
        bottom_bar_layout.setContentsMargins(8, 0, 8, 8)
        bottom_bar_layout.addWidget(self._scrub_bar)
    ...
    def _sync_scrub_bar(self) -> None:
        ...
        triggered it (Left/Right, the scrub bar's own drag, or the
        initial start_index) -- setValue() doesn't emit sliderMoved (only
        an actual drag does), so this never feeds back into
        go_to_index() and re-navigate. Kept as its own method, called
        from the same place _position_label gets updated, so the label
        and the scrub bar can never drift out of sync with each other.
        """
