...
def test_loads_before_and_after_frames_nearest_first(tmp_path, controller):
    ...
    assert before_layers[0] == (b"fake-png-2", pytest.approx(0.4))
    assert before_layers[1] == (b"fake-png-1", pytest.approx(0.2))

    assert len(after_layers) == 1
    assert after_layers[0] == (b"fake-png-4", pytest.approx(0.4))
