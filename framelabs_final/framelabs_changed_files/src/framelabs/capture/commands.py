...
class ReorderFramesCommand(Command):
    """Move frames elsewhere in the sequence (backlog item #2); undoable by
    reapplying the exact same move in reverse.

    Unlike Delete/Replace, this needs no on-disk backup copies at all --
    reorder_frames() only ever renames existing files (via
    _apply_frame_number_mapping()), it never creates or destroys any, so
    its own return value (the old-number -> new-number mapping it applied)
    is everything undo() needs to reverse it: just apply that mapping
    backwards. do() re-derives the mapping from frame_numbers/insert_before
    on every call (initial run AND every redo) rather than reusing a
    stored one, but that's still deterministic here -- undo() always
    leaves the project in exactly the state do() found it in, so redo()
    computing the same move from the same inputs always reproduces the
    same mapping. This mirrors ToggleFrameMarkerCommand's "no stored
    state, the operation is its own exact inverse" shape, just at the
    mapping level instead of a single boolean.
    """
    ...
    def __init__(
        ...
        """Prepare to move `frame_numbers` to just before `insert_before`.

        Args:
            project: The active project. Must have a non-None project_path.
            event_bus: The event bus FRAMES_REORDERED will be published on.
            frame_numbers: The frames to move -- see reorder_frames()'s own
                docstring for the exact semantics (order-independent, moved
                as a block in sequence order).
            insert_before: The frame number to insert before, or None to
                move to the end.
        """
        self._project = project
        self._event_bus = event_bus
        ...
        # Set by do() on every call, read by undo() to know what to
        # reverse. None only before do() has ever run.
        self._last_mapping: dict[int, int] | None = None

    @property
    def description(self) -> str:
        """Human-readable label, e.g. "Reorder 3 Frame(s)"."""
        return f"Reorder {len(self._frame_numbers)} Frame(s)"

    def do(self) -> None:
        """Move the frames, renumbering whatever shifts as a result."""
        self._last_mapping = reorder_frames(
            self._project, self._event_bus, self._frame_numbers, self._insert_before
        )

    def undo(self) -> None:
        """Reverse the most recent do()'s renumbering exactly.
        ...
        if self._last_mapping is None:
            raise RuntimeError(
                "ReorderFramesCommand.undo() called before do() -- "
                "nothing to undo yet."
            )
        inverse = {new: old for old, new in self._last_mapping.items()}
        if inverse:
            _apply_frame_number_mapping(self._project, inverse)
            ProjectSerializer.save(self._project)
            self._event_bus.publish("FRAMES_REORDERED", {"mapping": inverse})
            logger.info("Reverted reorder of %d frame(s) via undo", len(inverse))
