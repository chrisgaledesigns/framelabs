...
class FrameThumbnail(QFrame):
    ...
    reorder_dragged = Signal(int, int)
    ...
    def mouseMoveEvent(self, event) -> None:  # noqa: N802 (Qt override name)
        ...
        selection (`_drag_eligible`), crossing the threshold instead
        starts a reorder-drag -- `reorder_dragged` fires with this
        thumbnail's index and the current global x, and TimelineWidget
        (which alone knows every thumbnail's on-screen geometry) turns
        that into a drop-position indicator. An unselected thumbnail keeps
        the original drag-to-scroll behavior unchanged, so the existing
        gesture still works everywhere it always did.
        """
        if (
            ...
            if self._dragging:
                if self._reordering:
                    self.reorder_dragged.emit(self._index, int(current_x))
                else:
    ...
    def mouseReleaseEvent(self, event) -> None:  # noqa: N802 (Qt override name)
        """Emit `clicked` if this gesture ended as a click, or
...
class TimelineWidget(QScrollArea):
    ...
    def _on_reorder_dragged(self, dragged_index: int, global_x: int) -> None:
        """Move the drop-position indicator to match an in-progress
        reorder-drag (backlog item #2).
