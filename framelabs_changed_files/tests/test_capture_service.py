...
# ---------------------------------------------------------------------------
# Backlog item #2: reorder_frames() (Timeline multi-select drag-to-reorder).
# ---------------------------------------------------------------------------


def _captured_frames(project, camera_manager, event_bus, count: int, notes_prefix="f"):
    """Capture `count` frames in order and label each with a distinct note,
    so tests can verify identity survives renumbering (a renumbered
    frame's notes travel with it -- only its number/file change)."""
    frames = [capture_frame(project, camera_manager, event_bus) for _ in range(count)]
    for i, frame in enumerate(frames):
        frame.notes = f"{notes_prefix}{i}"
    return frames


def _assert_frame_files_exist(project, number: int) -> None:
...
def test_reorder_frames_moves_single_frame_and_renumbers_between(tmp_path):
    project = _make_project(tmp_path)
    camera_manager = FakeCameraManager()
    event_bus, _ = _make_event_bus()
    _captured_frames(project, camera_manager, event_bus, 4)  # numbers 1..4

    reorder_received = []
    event_bus.subscribe(
        "FRAMES_REORDERED", lambda payload: reorder_received.append(payload)
    )

    mapping = reorder_frames(project, event_bus, [1], insert_before=4)

    assert mapping == {2: 1, 3: 2, 1: 3}
    ordered = sorted(project.frames, key=lambda f: f.number)
    assert [f.notes for f in ordered] == ["f1", "f2", "f0", "f3"]
    assert reorder_received == [{"mapping": mapping}]
    for number in (1, 2, 3, 4):
        _assert_frame_files_exist(project, number)


def test_reorder_frames_to_end_when_insert_before_is_none(tmp_path):
    project = _make_project(tmp_path)
    camera_manager = FakeCameraManager()
    event_bus, _ = _make_event_bus()
    _captured_frames(project, camera_manager, event_bus, 3)  # numbers 1..3

    mapping = reorder_frames(project, event_bus, [1], insert_before=None)

    ordered = sorted(project.frames, key=lambda f: f.number)
    assert [f.notes for f in ordered] == ["f1", "f2", "f0"]
    assert mapping == {2: 1, 3: 2, 1: 3}


def test_reorder_frames_preserves_sequence_order_of_moved_block(tmp_path):
    """Passing frame numbers in an arbitrary order (not sequence order)
    must still move them as a block in their existing sequence order."""
    project = _make_project(tmp_path)
    camera_manager = FakeCameraManager()
    event_bus, _ = _make_event_bus()
    _captured_frames(project, camera_manager, event_bus, 5)  # numbers 1..5

    # Move frames 2 and 4 (passed out of order) to the end.
    mapping = reorder_frames(project, event_bus, [4, 2], insert_before=None)

    ordered = sorted(project.frames, key=lambda f: f.number)
    assert [f.notes for f in ordered] == ["f0", "f2", "f4", "f1", "f3"]
    assert mapping == {3: 2, 5: 3, 2: 4, 4: 5}


def test_reorder_frames_no_op_returns_empty_mapping(tmp_path):
    project = _make_project(tmp_path)
    camera_manager = FakeCameraManager()
    event_bus, _ = _make_event_bus()
    _captured_frames(project, camera_manager, event_bus, 3)  # numbers 1..3
    before = [f.number for f in sorted(project.frames, key=lambda f: f.number)]
    ...
    # Frame 1 is already immediately before frame 2 -- this is a no-op.
    mapping = reorder_frames(project, event_bus, [1], insert_before=2)

    assert mapping == {}
    assert reorder_received == []
    after = [f.number for f in sorted(project.frames, key=lambda f: f.number)]
    assert after == before


def test_reorder_frames_missing_frame_number_raises_frame_not_found(tmp_path):
    project = _make_project(tmp_path)
    event_bus, _ = _make_event_bus()
    _captured_frames(project, FakeCameraManager(), event_bus, 2)

    with pytest.raises(FrameNotFoundError):
        reorder_frames(project, event_bus, [99], insert_before=None)


def test_reorder_frames_missing_insert_before_raises_frame_not_found(tmp_path):
    project = _make_project(tmp_path)
    event_bus, _ = _make_event_bus()
    _captured_frames(project, FakeCameraManager(), event_bus, 2)

    with pytest.raises(FrameNotFoundError):
        reorder_frames(project, event_bus, [1], insert_before=99)


def test_reorder_frames_insert_before_in_moving_set_raises_value_error(tmp_path):
    project = _make_project(tmp_path)
    event_bus, _ = _make_event_bus()
    _captured_frames(project, FakeCameraManager(), event_bus, 3)

    with pytest.raises(ValueError):
        reorder_frames(project, event_bus, [1], insert_before=1)


def test_reorder_frames_empty_frame_numbers_raises_value_error(tmp_path):
    project = _make_project(tmp_path)
    event_bus, _ = _make_event_bus()
    _captured_frames(project, FakeCameraManager(), event_bus, 2)

    with pytest.raises(ValueError):
        reorder_frames(project, event_bus, [], insert_before=None)
